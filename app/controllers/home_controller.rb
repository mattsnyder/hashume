...
class HomeController < ApplicationController
  expose :tweets, :screen_name, :mentions, :last_update, :hashtags
  before_action :update_tweets, only: :dashboard

  helper_method :profile_pic_url
  def profile_pic_url(name)
    self.mentions.detect{|m| m['name'].downcase == name.downcase}['profile_image_mini']
  end
  ...
  def dashboard
    ...
    self.hashtags = self.tweets.map{|t| t['hashtags'] }.flatten.uniq
  end
  ...
  def update_tweets
    Rails.logger.info "update_tweets"
